...
// The max number of times to retry a query
var MAX_QUERY_RETRIES = 3;
 ...
 */
PooledConnectionQuery.prototype.shouldRetry = function() {
...
PooledConnection.prototype.execute = function(query, args, executeCallback) {
  ...
  async.whilst(
    pooledQuery.shouldRetry.bind(pooledQuery),
    ...
    function finished(err) {
      if (!err && !pooledQuery.success && pooledQuery.attempts === MAX_QUERY_RETRIES) {
        self.emit('log', 'error', 'Giving up performing query after retries');
      }
      callback(err, pooledQuery.result, pooledQuery.metadata);
    }
  );
};
