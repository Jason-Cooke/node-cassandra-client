...
PooledConnection.prototype.execute = function(query, args, executeCallback) {
  ...
  async.whilst(
    ...
    function finished(err) {
      if (!err && !pooledQuery.success && pooledQuery.attempts === self.max_query_retries) {
        self.emit('log', 'error', 'Giving up performing query after ' + self.max_query_retries + ' retries.');
      }
