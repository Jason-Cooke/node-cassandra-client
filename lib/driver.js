...
PooledConnection.prototype.execute = function(query, args, executeCallback) {
  var self = this;
  return self._execute(query, args, executeCallback);
};
 ...
 *    DELETE             : callback(err, undefined, metadata)
 */
PooledConnection.prototype._execute = function(query, args, callback) {
  var self = this,
      retryCount = 0,
  ...
  async.whilst(
    function test() {
      return executed === false && retryCount < retryLimit;
    },

    tryExecuteOnce.bind(null),

    function finished(err) {
      if(err) {
        self.emit('log', 'error', 'could not execute the query due to error', {err: err});
        wrappedCallback(err);
      } else if(retryCount === retryLimit && executed === false) {
        self.emit('log', 'error', 'could not execute the query ' + query + ' after ' + retryCount + ' of retries',
                  {err: err});
        wrappedCallback(new Error("Failed execution even after retries"));
      }
      return;
    }
  );
